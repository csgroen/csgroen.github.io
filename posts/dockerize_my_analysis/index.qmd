...
Now we need to create our docker file. There's a lot more information about it [here](https://docs.docker.com/develop/develop-images/dockerfile_best-practices/) and a cheat sheet on docker [here](https://collabnix.com/docker-cheatsheet/).
...
Time to assemble the `Dockerfile` and build our image.

For all R projects, look no further than the [Rocker project](https://www.rocker-project.org/images/) for all your base image needs. What the best version will be depends on this project, but my general direction is: if you use tidyverse anywhere, use the tidyverse image. Otherwise, use the rstudio image to have a GUI into your environment.

Now, make sure you take a rocker image with the same R version you are running locally. This base Dockerfile should work with minimal changes between projects:
...
Here, we are mounting the `example` directory into the `/project` directory of our container and preparing the password and username so we can use Rstudio server to connect to the image.
...
Here, we see our `project` folder in the Files. Navigate there and you'll see our `sleep.R` script. Open it and run it.
...
Now that everything works, we can push our docker, and add the example (with the code and data) to Github. In this minimal example, we don't have any additional data other than the script, but you could add any relevant data that as well.
